...
function VisualisationAbonnement() {
  ...
  useEffect(() => {
    ...
    // Mettre en place l'intervalle pour rafraîchir toutes les 3 secondes
    const intervalId = setInterval(fetchAbonnements, 3000);

    // Nettoyer l'intervalle lorsque le composant est démonté
    return () => clearInterval(intervalId);
  }, []); // L'intervalle sera configuré une seule fois lors du montage du composant
  ...
  const handleEnregistrerEdition = async () => {
    try {
      ...
      console.log("editionAbonnement:", editionAbonnement);
      setEditionAbonnement(null);
    } catch (error) {
      console.error(
        "Erreur lors de l'enregistrement des modifications de l'abonnement :",
        error
      );
    }
  };
  ...
  return (
    <div className="container mx-auto mt-8">
      <div className="container mx-auto p-4 max-w-md mx-auto bg-white p-6 rounded-3xl shadow-md">
        <h1 className="text-2xl font-bold mb-4">Abonnements disponibles</h1>
